...
// RealSerialPort 真实串口实现（Windows 版本）
// 使用 go.bug.st/serial 库，该库支持 Windows 平台
type RealSerialPort struct {
	mu     sync.Mutex
	port   serial.Port
	open   bool
	path   string // COM1, COM2, COM3, ...
	config *Config
}

// NewRealSerialPort 创建真实串口（Windows 版本）
...
// NewPort 创建串口（Windows 版本，优先使用真实实现）
